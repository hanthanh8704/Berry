...
public interface SanPhamRepositoty extends JpaRepository<SanPham,Integer> {
    ...
    List<ChiTietSanPham> findAllSPCTBySanPhamId(@Param("idSP") Integer idSP);

    @Query("SELECT anh FROM Anh anh " +
            "JOIN FETCH anh.chiTietSanPham spct " +  // LEFT JOIN FETCH để tải dữ liệu SPCT cùng với ảnh
            "WHERE spct.id = :idSPCT")
    List<Anh> findAllAnhBySanPhamCTId(@Param("idSPCT") Integer idSPCT);
