...
public class CartController {
    ...
    @GetMapping("/voucher/{idKH}")
    public ResponseEntity<List<Voucher>> getAllVoucher(@PathVariable("idKH") Integer idKH) {
        List<Voucher> phieuGiamGiaList = gioHangService.getAllVoucher(idKH);
        return ResponseEntity.ok(phieuGiamGiaList);
    }
    @GetMapping("/auto/select/voucher")
    public ResponseEntity<Voucher> autoSelectVoucher(@RequestParam("price") BigDecimal price, @RequestParam("idKH") Integer idKH) {
        Voucher selectedVoucher = gioHangService.autoSelectVoucher(price, idKH);
        Voucher voucher = new Voucher();
        if (selectedVoucher == null) {
            return ResponseEntity.ok(voucher); // Trả về mảng rỗng nếu không tìm thấy voucher
        }

        return ResponseEntity.ok(selectedVoucher); // Trả về voucher phù hợp
    }


    @GetMapping("/orders/{idKH}")
    public List<BillRequestClient> getAllDonMuaByIdKh(@PathVariable("idKH") Integer idKH) {
