...
public class CartServiceImpl implements CartService {
    ...
    @Autowired
    ProductDetailRepository sanPhamChiTietRepository;
    @Autowired
    VoucherCustomerRepositoryClient voucherCustomerRepositoryClient;
    @Autowired
    MailUtils mailUtils;
    ...
    public List<CartDetailRequest> getAll(Integer idKH) {
        ...
        for (CartDetail gioHangChiTiet : gioHangChiTietList) {
            ProductDetail p = sanPhamChiTietRepository.findById(gioHangChiTiet.getProductDetail().getId()).get();
            ...
            gioHangChiTietRequest.setPrice(p.getDiscountPrice().compareTo(BigDecimal.ZERO) == 0 ? p.getPrice() : p.getDiscountPrice());
...
//            gioHangChiTietRequest.setUpdated_at(gioHangChiTiet.getNguoiSua());
    ...
    public ResponseObject create(CartRequest request) {
        ...
        if (existingGioHang != null) {
            // Tìm chi tiết giỏ hàng theo sản phẩm và giỏ hàng
    ...
    public CartRequest muaHang(CartRequest request) {
        ...
        // Lấy giỏ hàng chính của khách hàng (nếu tồn tại)
        Cart gioHangCu = gioHangRepository.findByKhachHangId(request.getCustomerId());

        // Trường hợp "mua ngay" - khi không có danh sách chi tiết giỏ hàng
        if (request.getSpctId() != null) {
            ProductDetail productDetail = sanPhamChiTietRepository.findById(request.getSpctId()).get();

            if (productDetail != null) {
                CartDetail cartDetail = new CartDetail();
                cartDetail.setProductDetail(productDetail);
               // cartDetail.setPrice(productDetail.getPrice());
                cartDetail.setCart(gioHangCu);
                cartDetail.setQuantity(request.getQuantity());
                gioHangChiTietRepository.save(cartDetail);
                CartDetail chiTiet2 = gioHangChiTietRepository.findById(cartDetail.getId()).get();
                chiTietListAo.add(chiTiet2);
            }
        } else {
            // Trường hợp thêm sản phẩm từ giỏ hàng chi tiết
            for (Integer id : request.getIdGHCT()) {
                gioHangChiTietRepository.findById(id).ifPresent(chiTiet -> {
                    ProductDetail spct = spct_repository.findById(chiTiet.getProductDetail().getId()).orElse(null);
        ...
        gioHangAo.setId(gioHangCu.getId());  // ID giữ nguyên để liên kết với giỏ hàng cũ
        ...
        gioHangAo.setStatus("Hoạt động");  // Giỏ hàng ảo với trạng thái mới
    ...
    public ResponseObject thanhToan(BillRequestClient request) {
        ...
        if (request.getVoucherId() != null) {
            Voucher voucher = phieuGiamGiaRepository.findById(request.getVoucherId()).orElse(null);
            if (voucher != null) {
                // Thiết lập các thuộc tính khác cho hóa đơn
                hoaDon.setVoucher(voucher);
            } else {
                System.out.println("Voucher không tồn tại hoặc không hợp lệ.");
            }
        } else {
            hoaDon.setVoucher(null); // Đơn hàng không có voucher
        }
...
//        hoaDon.setUpdatedBy(null);
        ...
        if (request.getVoucherId() != null) {
            ...
            Optional<Voucher> optionalVoucher = phieuGiamGiaRepository.findById(request.getVoucherId());
            if (optionalVoucher.isPresent()) {
                ...
                // Nếu số lượng bằng 0, đánh dấu là đã xóa
                if (newQuantity == 0) {
                ...
                // Xử lý theo từng loại voucher
//                if (optionalVoucher.get().getType() == VoucherConstant.CA_NHAN) {
//                    // Lấy và xóa bản ghi của khách hàng nếu có
//                    Optional<VoucherCustomer> voucherCustomer = voucherCustomerRepositoryClient.findByIdVoucherAndIdKH(optionalVoucher.get().getId(), request.getCustomerId());
//                    if (voucherCustomer != null) {
//                        voucherCustomerRepositoryClient.delete(voucherCustomer.get());
//                    }
//                }
            } else {
        ...
        for (Integer x : request.getIdSPCTs()) {
            ...
            hoaDonChiTiet.setPrice(gioHangChiTiet.getProductDetail().getPrice());
            hoaDonChiTiet.setPromotion(gioHangChiTiet.getProductDetail().getDiscountPercentage());
...
//            spct_repository.save(sanPhamCT);
        ...
        try {
            String qrCodeText = hoaDon.getCode();
...
//                getListSPCT
    ...
    @Override
    public List<Voucher> getAllVoucher(Integer idKH) {
        // Lấy thời gian hiện tại
        LocalDateTime ngayHienTai = LocalDateTime.now();

        // Lấy danh sách voucher cá nhân của khách hàng theo idKH và ngày hiện tại
        List<Voucher> caNhanVouchers = phieuGiamGiaRepository.findAllByCaNhan(idKH, VoucherConstant.CA_NHAN, ngayHienTai);

        // Lấy danh sách voucher cộng đồng (public) có hiệu lực
        List<Voucher> congDongVouchers = phieuGiamGiaRepository.findAllByPublic(ngayHienTai, VoucherConstant.CONG_KHAI);

        // Kết hợp cả hai danh sách
        List<Voucher> allVouchers = new ArrayList<>();

        allVouchers.addAll(caNhanVouchers);
        allVouchers.addAll(congDongVouchers);

        return allVouchers;
    }

    @Override
    public Voucher autoSelectVoucher(BigDecimal price, Integer idKH) {
        // Kiểm tra nếu giá trị price bằng 0 thì trả về null ngay lập tức
        if (price.compareTo(BigDecimal.ZERO) == 0) {
            return null; // Không áp dụng voucher nếu price = 0
        }
        ...
        // Lấy danh sách voucher cá nhân của khách hàng theo idKH và ngày hiện tại
        List<Voucher> caNhanVouchers = phieuGiamGiaRepository.findAllByCaNhanPhuHop(
                idKH, VoucherConstant.CA_NHAN, ngayHienTai, price);

        // Lấy danh sách voucher cộng đồng (public) có hiệu lực
        List<Voucher> congDongVouchers = phieuGiamGiaRepository.findAllByCongKhaiPhuHop(
                ngayHienTai, VoucherConstant.CONG_KHAI, price);
        ...
        // Trả về voucher đầu tiên nếu tồn tại, hoặc null nếu danh sách rỗng
        return allVouchers.isEmpty() ? null : allVouchers.get(0);
    }
