...
public class VoucherServiceImpl implements VoucherService {
    ...
    private void sendVoucherEmail(Customer customer, Voucher voucherSave) {
        String emailContent = "<!DOCTYPE html>"
                ...
                + "    <div style='background-color: #007BFF; color: white; padding: 15px; text-align: center; font-size: 1.5em;'>"
                + "        Chúc mừng bạn đã nhận được phiếu giảm giá!"
                + "    </div>"
                + "    <div style='padding: 20px;'>"
                + "        <p>Chào <span style='color: #007BFF; font-weight: bold;'>" + customer.getFullName() + "</span>,</p>"
                + "        <p>Chúng tôi rất vui mừng thông báo rằng bạn đã nhận được phiếu giảm giá từ <b>Berry Store</b>!</p>"
                + "        <p>Dưới đây là thông tin chi tiết về phiếu giảm giá của bạn:</p>"
                + "        <ul style='list-style-type: none; padding: 0;'>"
                + "            <li><b>Mã phiếu giảm giá:</b> <span style='color: #007BFF; font-weight: bold;'>" + voucherSave.getCode() + "</span></li>"
                + "            <li><b>Giá trị giảm giá:</b> " + voucherSave.getDiscountValue() + "%</li>"
                + "            <li><b>Ngày hết hạn:</b> " + voucherSave.getEndDate() + "</li>"
                + "            <li><b>Điều kiện áp dụng:</b></li>"
                + "            <ul style='list-style-type: disc; margin-left: 20px;'>"
                + "                <li>Giá trị tối thiểu: " + voucherSave.getMinOrderValue() + "</li>"
                + "                <li>Giá trị tối đa: " + voucherSave.getMaxDiscountValue() + "</li>"
                + "            </ul>"
                + "        </ul>"
                + "        <p>Hãy sử dụng mã này để được giảm giá khi mua sắm tại cửa hàng hoặc trên trang web của chúng tôi.</p>"
                + "        <p style='font-size: 0.9em; color: #777;'>Đây là email tự động, vui lòng không trả lời email này.</p>"
                + "    </div>"
                + "    <div style='text-align: center; font-size: 0.9em; color: #555; padding: 15px; background-color: #f4f4f4;'>"
                + "        Cảm ơn bạn đã ủng hộ <b>Berry Store</b>.<br>"
                + "        Liên hệ: <a href='https://www.facebook.com/ninh.cong.9889' style='color: #007BFF; text-decoration: none;'>Berry Store trên Facebook</a>"
                + "    </div>"
                + "</div>"
                + "</body>"
                + "</html>";
    ...
    public Voucher add(VoucherRequest request) {
        ...
        if (request.getType() ==VoucherConstant.CONG_KHAI) {
        ...
        if (request.getDiscountMethod().equals("PHAN_TRAM")) {
            try {
                ...
                if (request.getDiscountValue().compareTo(BigDecimal.ZERO) <= 0 || request.getDiscountValue().compareTo(maxValue) > 0) {
                    throw new RestApiException("Phần trăm giảm phải nằm trong khoảng từ 1 đến 100!");
                }
        ...
        if (voucherSave.getType() == VoucherConstant.CA_NHAN) {
    ...
    public Voucher update(Integer id, VoucherRequest request) {
        ...
        if (request.getType() == VoucherConstant.CONG_KHAI) {
            ...
            if (request.getQuantity() > 10000) {
                throw new RestApiException("Số lượng không được vượt quá 10,000!");
            }
        }
...
//        OffsetDateTime currentDateTime = OffsetDateTime.now(ZoneOffset.UTC);
    ...
    public Voucher updateEndDate(Integer id) {
        ...
        List<VoucherCustomer> customerVouchers = accountVoucherRepository.findByIdPhieuGiamGiaAndTrangThai(savedVoucher.getId(), StatusVoucher.DA_KET_THUC);
    ...
    @Override
    public Voucher updateStartStatus(Integer id) {
        Voucher voucherToUpdate = voucherRepository.findById(id).orElse(null);
        if (voucherToUpdate == null) {
            throw new RestApiException("Phiếu giảm giá không tồn tại.");
        }
        voucherToUpdate.setStatus(VoucherConstant.DANG_SU_DUNG);
        // Lưu voucher và gửi email thông báo
        Voucher savedVoucher = voucherRepository.save(voucherToUpdate);
        // Lấy danh sách khách hàng có trạng thái voucher "Chưa sử dụng"
        return savedVoucher;
