import { Button, Tooltip, Collapse, Empty, Input, InputNumber, Modal, Upload, message, Checkbox } from "antd";
...
function TableProduct({ props, handleChange }) {
    ...
    const handleChangeQuantity = (value, colorName, index) => {
        // Kiểm tra nếu giá trị trống hoặc < 1
        if (!value || value < 1) {
            message.error("Số lượng phải >= 1 và không được để trống!");
            return; // Dừng lại không thực hiện các thao tác tiếp theo
        }
    ...
    const handleChangePrice = (event, colorName, index) => {
        ...
        // Kiểm tra nếu giá trị trống hoặc < 1
        if (!value || value < 1) {
            message.error("Đơn giá phải >= 1 và không được để trống!");
            return; // Dừng lại không thực hiện các thao tác tiếp theo
        }
    ...
    const handleChangeWeight = (event, colorName, index) => {
        ...
        // Kiểm tra nếu giá trị trống hoặc < 1
        if (!value || value < 1) {
            message.error("Cân nặng phải >= 1 và không được để trống!");
            return; // Dừng lại không thực hiện các thao tác tiếp theo
        }
                            ...
                            <tbody className="text-center">
                                {Object.entries(groupByColor).map(([key, items], index) => (
                                    <React.Fragment key={index}>
                                            ...
                                            <tr key={idx}>
                                                {option.product && option.product !== undefined && option.product !== null ? (
                                                    <>
                                                        ...
                                                        <td>
                                                            {option.product === undefined || option.product === null
                                        ...
                                        <tr>
                                            <td title="Thêm ảnh" colSpan={11} className="text-center">
