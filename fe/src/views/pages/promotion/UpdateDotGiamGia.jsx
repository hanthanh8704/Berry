...
const UpdateDotGiamGia = () => {
    ...
    if (idDGG) {
      getDotGiamGiaDetailByIdDGG(idDGG)
        .then((response) => {
          ...
          console.log('san pham chi tiet bien the ', response.data)
        })
        .catch((error) => {
          console.error('Error fetching dot giam gia data:', error);
        });
        ...
        Modal.confirm({
          ...
          onOk: async () => {
            ...
            try {
              ...
              setTimeout(() => {
                navigate('/promotion'); //Sau 2s mới chuyển
              }, 2000);
            } catch (error) {
              toast.error(error);
            }
  ...
  // Tự động chọn tất cả sản phẩm khi vào trang sửa
  useEffect(() => {
    if (!selectedSanPhamIds || selectedSanPhamIds.length === 0) {
      const allIds = sanPhamCT.map(gh => gh.id);
      setSelectedSanPhamDetailIds(allIds);
    }
  }, [sanPhamCT, selectedSanPhamIds]);

  return (
    <Paper>
      <div style={{ backgroundColor: 'white', border: '1px solid black', borderRadius: '5px' }} className='px-3 py-2'>
        <h5>Sửa đợt giảm giá</h5>
      </div>
      <Box
        display="flex"
