...
function AddVoucher() {
  ...
  const [discountMethod, setDiscountMethod] = useState('PHAN_TRAM');
  ...
  return (
    ...
    <div className="container">
      <Form onFinish={handAddVoucher} layout="vertical" form={form}
        tialValues={{ quantity: type === 'CA_NHAN' ? 1 : undefined }}
            ...
            <Row gutter={10}>
              ...
              <Col xl={12}>
                <Form.Item label="Giá trị giảm" name="discountValue" rules={[
                  ...
                  <Input type="number" min={0} placeholder={`Nhập giá trị giảm (${discountMethod})...`} suffix={discountMethod == 'PHAN_TRAM' ? '%' : 'VNĐ'} />
                </Form.Item>
              </Col>
              <Col xl={12}>
              ...
              <Col xl={12}>
                <Form.Item label={"Loại phiếu giảm giá"} name={"type"} rules={[{ required: true, message: "Loại phiếu giảm giá không được để trống!" }]}>
  ...
  >
    <Input 
      ...
      disabled={type === 'CA_NHAN'} 
      value={type === 'CA_NHAN' ? 1 : undefined} 
      onChange={(e) => {
        if (type !== 'CA_NHAN') {
