...
const ProductFemale = () => {
    ...
    const fetchFilteredProducts = () => {
        getSearchProductsKCTHMS(selectedMauSac, 2, selectedThuongHieu, selectedKichCo, selectedPriceRange, sort)
            .then(response => {
    ...
    const getAllSanPhamByMale = () => {
        axios
            .get(`http://localhost:8080/api/client/products/female`)
            .then((response) => {
                setProduct(response.data);
                console.log('producteeeeeeeeeeeeeeeeee male ', response.data);
            })
            .catch((error) => {
                console.error('Error fetching San Pham CT:', error);
                setProduct([]);
